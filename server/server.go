...
// AddProvider registers the provider with the server so that the types that it
// provides are available during requests.
func (s *Server) AddProvider(p a.Provider) {
	if s.services == nil {
		s.services, _ = di.New(context.Background())
	}
	s.services.Provide(p)
}
